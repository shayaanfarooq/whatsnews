...
// used in personal feed for a category row

export const useCategoryFeedData = (sources: string[], params: ContentParams) => {
      ...
      queries: finalSources.map((source): UseQueryOptions<ContentResponse, unknown, Article[]> => {
         switch (source) {
            case NewsSource.NewsApi:
               // news api
               return {
                  queryKey: [queryKeys.newsApiPersonalFeed, params],
            ...
            case NewsSource.Guardian:
               // guardian
               return {
                  queryKey: [queryKeys.guardianPersonalFeed, params],
